...
> **Note:** `APPLICATIONINSIGHTS_CONNECTION_STRING` is intentionally omitted from
...
> **Single-pass deployment:** The deploy script creates the App Registration via `az ad app`
> commands **before** Bicep runs, so all parameters are available in a single deployment pass.
> No client secret is needed — Easy Auth validates tokens using only the client ID and
> OIDC metadata. The Managed Identity handles all Microsoft Graph API calls.
